...
begin
  ...
  if target_program_id is not null and exists (
    ...
    where not exists (
      ...
      join public.programs enrolled_program
        on enrolled_program.organization_id = enrolled_cohort.organization_id
        and enrolled_program.id = enrolled_cohort.program_id
      where se.organization_id = target_organization_id
        and se.startup_id = requested.startup_id
        and enrolled_program.incubator_id = target_incubator_id
        and enrolled_cohort.program_id = target_program_id
        and enrolled_program.deleted_at is null
        and enrolled_cohort.deleted_at is null
  ...
  with participants as (
    ...
    select
      target_organization_id, target_incubator_id, created_campaign_id,
  ...
  select
    ...
    p.evaluator_id, created_campaign_id, p.id, campaign_ends_at
  from participants p;
  ...
  select
    ...
    jsonb_build_object('campaign_id', created_campaign_id, 'campaign_name', btrim(campaign_name))
...
revoke execute on function public.create_diagnostic_campaign(
  uuid, uuid, text, timestamptz, timestamptz, uuid[], uuid, uuid, uuid, text, text, text
) from public, anon;
grant execute on function public.create_diagnostic_campaign(
  uuid, uuid, text, timestamptz, timestamptz, uuid[], uuid, uuid, uuid, text, text, text
) to authenticated;

comment on function public.create_diagnostic_campaign(
  uuid, uuid, text, timestamptz, timestamptz, uuid[], uuid, uuid, uuid, text, text, text
) is 'Cria campanha, participantes e aplicações em uma única transação com escopo validado no banco.';
